...
public class EvaluateBar extends View {
    ...
    private boolean integerMark = false;    //设置是否需要整数评分
    private boolean isTouch = true; //是否允许调动分数


    public EvaluateBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public EvaluateBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        setClickable(true);
        TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.EvaluateBar);
        this.starDistance = (int) mTypedArray.getDimension(R.styleable.EvaluateBar_starDistance, 0);
        this.starSize = (int) mTypedArray.getDimension(R.styleable.EvaluateBar_starSize, 20);
        this.starCount = mTypedArray.getInteger(R.styleable.EvaluateBar_starCount, 5);
        this.starEmptyDrawable = mTypedArray.getDrawable(R.styleable.EvaluateBar_starEmpty);
        this.starFillBitmap = drawableToBitmap(mTypedArray.getDrawable(R.styleable.EvaluateBar_starFill));
        this.starMark  = mTypedArray.getFloat(R.styleable.EvaluateBar_starMark,0f);
        this.isTouch = mTypedArray.getBoolean(R.styleable.EvaluateBar_touch, true);
        mTypedArray.recycle();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                int i = (int) (x * 1.0f / (getMeasuredWidth() * 1.0f / starCount) + 0.9);
                setStarMark(i);
                break;
            }
