...
public class CxxAdddressPicker extends LinkagePicker {
    private OnAddressPickListener onAddressPickListener;
    private boolean hideProvince = false;
    private boolean hideCounty = false;
    private ArrayList<Province> mDatas;
     ...
     * @param data     the data
     */
    public CxxAdddressPicker(Activity activity, ArrayList<Province> data) {
        super(activity);
        this.mDatas = data;
    ...
    public void onSubmit() {
        if (onAddressPickListener != null) {
            if (hideCounty) {
                //地址
                String address = selectedFirstText + selectedSecondText + selectedThirdText;
                //省Id
                String province_id = mDatas.get(selectedFirstIndex).getAreaId();
                //城市Id
                String city_id = mDatas.get(selectedFirstIndex).getCities().get(selectedSecondIndex).getAreaId();
                onAddressPickListener.onAddressPicked(selectedFirstText,selectedSecondText,null, province_id, city_id,null);
            } else {
                //地址
                //省Id
                String province_id = mDatas.get(selectedFirstIndex).getAreaId();
                ...
                String city_id = mDatas.get(selectedFirstIndex).getCities().get(selectedSecondIndex).getAreaId();
                //城市Id
                String area_id = mDatas.get(selectedFirstIndex).getCities().get(selectedSecondIndex).getCounties().get(selectedThirdIndex).getAreaId();
                onAddressPickListener.onAddressPicked(selectedFirstText,selectedSecondText,selectedThirdText,province_id,city_id,area_id);
            }
        }
    }
    ...
    public void setItemString(String address_name){
        for (int i = 0; i < firstList.size(); i++) {
            String ft = firstList.get(i);
            if (address_name.contains(ft)) {
    ...
    public interface OnAddressPickListener {
         ...
         * On address picked.
         *
         * @param province the province_name
         *
         * @param province_id the province_id
         * @param city_id    the city
         * @param county_id  the county ，if {@hideCounty} is true，this is null
         */
        void onAddressPicked(String province,String city, String area,String province_id ,String city_id, String county_id);
